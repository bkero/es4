...
CheckItems(A,E,"sort()");
E = A.sort(Compare);
CheckItems(A,E,"sort(fn)");
E = Array.sort(A);
CheckItems(A,E,"Array.sort()");
E = Array.sort(A,Compare);
CheckItems(A,E,"Array.sort(fn)");
...
A = new Array( true, false, new Boolean(true), new Boolean(false), 'true', 'false' );
E = A.sort();
CheckItems(A,E,"sort()");
E = A.sort(Compare);
CheckItems(A,E,"sort(fn)");
...
A = new Array(); A[3] = 'undefined'; A[6] = null; A[8] = 'null'; A[0] = void 0;
E = Array()
//E = A.sort();
CheckItems(A,E,"sort()");
A = new Array();
...
function CheckItems( A, E, desc) {
  print("CheckItems "+A.toString()+" "+E.toString());
  new TestCase(   SECTION,
		  desc+" var A="+A.toString()+";  A.sort(); A.length",
		  E.length,
		  A.length);
  new TestCase(
      SECTION,
      desc+ " comparing sorted array to ["+E.toString()+"]",
      E.toString(),
      A.toString());
