...
\newcommand{\consistent}[2]{#1 \leadsto #2}
\newcommand{\csubtype}[2]{#1 <\leadsto #2}
\newcommand{\csupertype}[2]{#1 >\leadsto #2}

\newcommand{\consubtypeword}[1]{\,\,<\!\!\!{\tiny{#1}}\,\,}
\renewcommand{\consubtypeword}[1]{\,\,<\!\!_{\tiny{#1}}\,\,}
\newcommand{\consubtyped}[3]{#2 \consubtypeword{#1} #3}
\newcommand{\consubtype}[2]{#1 \consubtypeword{\top} #2}
%\newcommand{\consubtypem}[2]{#1 \consubtypeword{\bot} #2}
\newcommand{\notd}[1]{\overline #1}
...
The type system supports the usual subtype relation $\subtype S T$.
In addition, to support a notion of gradual typing, we also have a consistency relation $\consistent S T$,
which holds provided that $S$ and $T$ are identical except that $T$ may include $dynamic$ in places where $S$ does not. The \emph{consistent-subtype} relation $\csubtype S T$ is then the composition of these two relations: $\csubtype S T$ hold if there exists $U$ such that $\subtype S U$ and $\consistent U T$.
This relation is defined more directly in Figure~\ref{} via mutual recursion with an associated \emph{consistent-supertype} relation $\csupertype S T$.

Unlike the consistency relation of earlier work~\cite{SiekTaha}, our consistency relation is asymmetric (since $\consistent \dynamic \Int$ does not hold), but consistency and consistent-subtyping are both transitive.
 
\begin{lemma}
Subtyping, consistency, consistent-subtyping, and consistent-supertyping are transitive.
\end{lemma}
PROVEME
...
\[
\begin{array}{rcl}
	(\lamt{S}{T})  		&\csubtype{}{}&  (\lamt\dynamic\dynamic) \\
	\objty{l:T,\dots}		& \csubtype{}{}&  \objty{l:\dynamic} \\
	\objty{\dots}	   	& \csubtype{}{}&  \objty{} \\
\end{array}
\]
...
\begin{lemma}
If $\csubtype V {S}$ and $S\Implies T$ and $\csupertype U T$ then $\csubtype V U$.
\end{lemma}
