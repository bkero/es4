...
Unlike the consistency relation of earlier work~\cite{SiekTaha}, our compatibility relation is asymmetric (since $\compatible \dynamic \Int$ does not hold), but is transitive.

The \emph{compatible-subtyping} relation $\comsubtype S T$ is then the composition of these two relations: $\comsubtype S T$ hold if there exists $U$ such that $\subtype S U$ and $\compatible U T$.
...
\begin{lemma}
Subtyping, compatibility, and compatible-subtyping  are transitive.
\end{lemma}
PROVEME
...
If $\judges{\emptyset}{e}{T}$ then $\safe{}$ operations in $e$ can never get stuck.
\end{lemma}
PROVEME
...
\begin{array}{@{}rcllr}
	\sigma,C[{\app{(\wrapv{w}{(\lamt{S}{T})})}
	              {v}}]
	&\lred&
	\sigma,C[\cast{(w~(\cast{v}{S}))}{T}]
	&\rel{E-Beta-w}
\\
	\sigma,C[\objget{(\wrapv{w}{\objty{l:T,\dots}})}{l}]
	&\lred&
	\sigma,C[\cast{(\objget{w}{l})}{T}] 
	&\rel{E-Get-w}
\\
	\sigma,C[\objset{(\wrapv{w}{\objty{l:T,\dots}})}{l}{v}]
	&\lred&
	\sigma,C[\objset{w}{l}{\convert{v}{T}}]
	&\rel{E-Assign-w} \\
\end{array}
\]
