...
These check marks are inferred by an analysis in a later section; for now we mostly ignore them, and omit them if they are irrelevant.
...
\trule{Com-Obj}{
		...
		\compatible{\objty{l_i:S_i^{i\in 1..n+m}}}
				{\objty{l_i:T_i^{i\in 1..n}}}
	}
\end{trules}
\end{displayfigure}
...
Unlike the consistency relation of earlier work~\cite{SiekTaha}, our compatibility relation is asymmetric (since $\compatible \Int \dynamic $ but $\dynamic\not \compatible{}{} \Int$), but is transitive. 
...
We first extend the operational semantics to omit \emph{convert} checks on operations labelled with the check mark $\safe$:
...
If $\judges{\emptyset}{e}{T}$ then operations in $e$ marked with $\safe$ can never get stuck.
\end{lemma}
PROVEME
...
\begin{displayfigure}{th}{Consistency}
\label{fig:subtype}
\begin{trules}
...
\trule{Con-Obj}{
		...
		\consistent{\objty{l_i:S_i^{i\in 1..n+m}}}
				{\objty{l_i:T_i^{i\in 1..n}}}
	}
\end{trules}
\end{displayfigure}
...
Note that the type $T$ \emph{only} indicates that $e$ \emph{is intended to} produce values of type $T$.
Note that there are no guarantees here; the purpose of the strict mode type system is only to heuristically detect errors at verification time.

The \emph{consistency} relation $\consistent S T$ checks if types $S$ and $T$ are identical, where $\dynamic$ in either $S$ or $T$ matches any type. Thus, consistency extends  the compatibility relation, and it is symmetric but not transitive.
The \emph{consistent-subtyping} relation $\consubtype S T$ holds if there exists $U$ such that $\subtype S U$ and $\consistent U T$. The type rules rely on this consistent-subtyping relation to see if an expression of static type $S$ can be passed to a context expecting a type $T$.
...
We now extend the language and type language:
\[
\begin{array}{llr}
...
For subtyping, like types are increasing and covariant.
...
We add the following evaluation rules:
\[
\begin{array}{@{}rcllr}
	\sigma,C[{\app{c}{(\wrapv{w}{(\lamt{S}{T})})}
	              {v}}]
	&\lred&
	\sigma,C[\cast{\app{\unsafe}{w}{(\cast{v}{S})}}{T}]
	&\rel{E-Beta-w}
\\
	\sigma,C[\objget{c}{(\wrapv{w}{\objty{l:T,\dots}})}{l}]
	&\lred&
	\sigma,C[\cast{(\objget{\unsafe}{w}{l})}{T}] 
	&\rel{E-Get-w}
\\
	\sigma,C[\objset{c}{(\wrapv{w}{\objty{l:T,\dots}})}{l}{v}]
	&\lred&
	\sigma,C[\objset{\unsafe}{w}{l}{\convert{v}{T}}]
	&\rel{E-Assign-w} \\
\end{array}
\]
...
We extend the $\consubtype{}{}$ relation with $\consubtype{\Int}{\Bool}$.

We add the type checking and optimization rules for wrap:
\begin{trules}
\trule{O-As}{
		\judges{E}{t}{S}%\qquad 
		%\compatible{S}{T}
...
\end{trules}
We believe the current system then fully supports \t{like} and \t{wrap} types, and implicit integer-to-boolean conversions.
All the necessary proofs still remain to be done.
...
\label{fig:subtype}
\begin{trules}
\headtrule{$\consubtypeÄ{S}{T}$}{Subtyping}
\trule{Sub-Refl}{
	}{
		\consubtype{T}{T}
	}
\trule{Sub-Dyn-Top}{
		...
		\consubtyped{\top}{T}{\dynamic}
	}
\trule{Sub-Dyn-Bot}{
	}{
		\consubtyped{\bot}{\dynamic}T
	}
\trule{Sub-Arrow}{
		%(\consubtype{T_1}{S_1}\mbox{ or }T_1=\dynamic)\\
		\consubtyped{\notd m}{T_1}{S_1} \qquad
		\consubtype{S_2}{T_2}
	}{
		\consubtype{(\lamt{S_1}{S_2})}{(\lamt{T_1}{T_2})}
	}
\trule{Sub-Obj}{
		\consubtyped{\notd m}{T_i}{S_i}\qquad
		\consubtype{S_i}{T_i} \qquad\mbox{for $i\in 1..n$}
	}{
		\consubtype{\objty{l_i:S_i^{i\in 1..n+m}}}
				{\objty{l_i:T_i^{i\in 1..n}}}
	}
\end{trules}
\end{displayfigure}
