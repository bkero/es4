...
\newcommand{\comeqtype}[2]{#1 =\!\leadsto #2}
\renewcommand{\comeqtype}[2]{#1 =\!\!*\,\, #2}
...
\trule{Com-Generic}{
		...
		\compatible{\Forall X S}{\Forall X T}
	}
\end{trules}
\end{displayfigure}
...
%Unlike the Bicompatibility relation of earlier work~\cite{SiekTaha}, o
...
%Similarly, the \emph{bicompatible-supertype} relation $\csupertype S T$ holds if there exists $U$ such that $S>U$ and $\compatible U T$. These two relations can be defined directly via mutual
...
We first extend the operational semantics to omit \emph{convert} checks on operations labelled with the check mark $\safe$: see Figure~\ref{fig:eval2}. Thus, even though the original rule \rel{E-Beta} can evaluate all applications, \rel{E-Beta-Safe} provides an optimized evaluation rule for a safe application.
...
\trule{O-Set-Unsafe}{
		\judges{E}{e_1}{S} \qquad
		\judges{E}{e_2}{T}
	}{
		\judges{E}{(\objset{\unsafe}{e_1}{l}{e_2})}{T}
	}
\trule{O-Generic-Safe}{
		\judges{E,X}{t}{T'} \qquad
		{T'} \Implies {T} 
	}{	
		\judges{E}{(\Lam{\safe}{X}{t}{T})}{(\Forall X T)}
	}
\trule{O-Generic-Unsafe}{
		\judges{E,X}{t}{T'}  
	}{	
		\judges{E}{(\Lam{\unsafe}{X}{t}{T})}{(\Forall X T)}
	}
...
\begin{displayfigure}{th}{Matching and Bicompatibility}
\label{fig:convert}
\begin{trules}
\headtrule{$\matches{S}{T}$}{Matching}
\trule{Match-Refl}{
		\subtype S U \qquad
		\bicompatible U T
	}{
...
\headtrule{$\bicompatible{S}{T}$}{Bicompatibility}
\trule{Bic-Dyn}{
	}{
		\bicompatible{T}{\dynamic}
	}
\trule{Bic-Dyn}{
	}{
		\bicompatible{\dynamic}{T}
	}
\trule{Bic-Refl}{
	}{
		\bicompatible{T}{T}
...
\trule{Bic-Arrow}{
		\bicompatible{S_1}{T_1} \qquad
		\bicompatible{S_2}{T_2}
	}{
		\bicompatible{(\lamt{S_1}{S_2})}{(\lamt{T_1}{T_2})}
	}
\trule{Bic-Obj}{
		\bicompatible{S_i}{T_i} \qquad\mbox{for $i\in 1..n$}
	}{
		\bicompatible{\objty{l_i:S_i^{i\in 1..n}}}
				{\objty{l_i:T_i^{i\in 1..n}}}
	}
\trule{Bic-Generic}{
		\bicompatible{S}{T}
	}{
		\bicompatible{(\Forall X S)}{(\Forall X T)}
	}
\end{trules}
\end{displayfigure}
...
The type system is based on the \emph{matching relation} $\matches{S}{T}$ (shown in Figure~\ref{fig:convert})
to see if an expression of static type $S$ can be passed to a context expecting a type $T$.
The relation $\matches S T$ holds if there exists $U$ such that $\subtype S U$ and $\bicompatible U T$. 
In turn, the \emph{bicompatibility relation} $\bicompatible U T$ checks if types $U$ and $T$ are identical, where $\dynamic$ in either $U$ or $T$ matches any type. Thus, bicompatibility extends the compatibility relation, and it is symmetric but not transitive.

TODO: When do we have guarantees?

\begin{displayfigure}{thp}{Type Rules for Strict Mode 1}
\label{fig:typerules1}
\[
	E ::= \emptyset ~|~ E,x:T ~|~ E,X
\]
\begin{trules}
\headtrule{$\judgeE{E}$}{Well-formed \underline{e}nvironments}
\trule{E-Empty}{
	}{
		\judgeE{\emptyset}
	}
...
\headtrule{$\judgeT{E}T$}{\underline{W}ell-formed types}
\trule{W-Int}{
		\judgeE E
	}{
		\judgeT{E}{\Int}
...
\trule{W-Obj}{
		...
		\judgeT E {\objty{l_i:T_i^{i\in 1..n}}}
	}
\trule{W-Dynamic}{
		\judgeE E
	}{
...
\begin{displayfigure}{thp}{Type Rules for Strict Mode 2}
...
\trule{T-Generic}{
		...
		\judge{E}{(\Lam {} X e T)}{(\Forall X T)}
	}
\trule{T-GenericApp1}{
		\judge{E}{t}{(\Forall X T)} \qquad
		\judgeT{E}{S}
	}{
		\judge{E}{\App{t}{S}}{T[X:=S]}
	}
\trule{T-GenericApp1}{
		\judge{E}{t}{\dynamic}\qquad
		\judgeT{E}{S} 
	}{
		\judge{E}{\App{t}{S}}{\dynamic}
	}
\end{trules}
\end{displayfigure}
...
\\
	...
	&&\rel{E-Assign-w} 
\\
	\sigma,C[{\App{(\wrapv{w}{(\Forall X T)})}{S}}] 
	&\lred&
	\sigma,C[\wrap{\App{w}{S}}{T[X:=S]}]
	&&\rel{E-Generic-w}
\end{array}
\]
...
\trule{Sub-Wrap-Cov}{
		...
		\subtype {\wrapty S} {\wrapty T}
	}
\end{trules}
%
We extend compatibility and Bicompatibility to \t{like} and \t{wrap} types.
%
\begin{trules}
%\headtrule{$\compatible{S}{T}$}{Compatibility}
\trule{Com-Like}{
...
\trule{Bic-Like}{
		...
		\bicompatible{\likety S}{\likety T}
	}
\trule{Bic-Wrap}{
		\bicompatible S T
	}{
		\bicompatible{\wrapty S}{\wrapty T}
	}
\end{trules} 
%
We extend the matching relation to allow int-to-boolean conversions:
%
\begin{trules}
\trule{Match-Int-Bool}{
	}{
...
\newcommand{\alg}[4]{#3 \sqsubset^{#1}_{#2} #4}

Figure~\ref{fig:alg} presents an algorithm for $\comsubtype{}{}$.
These rules are not quite syntax-directed, but ambiguities are resolved in favor of the more specific rule.
That is, \rel{A-Like-Cov} is preferred to \rel{A-Like-Inc}, and ditto \rel{A-Wrap-Cov} over \rel{A-Wrap}.

We believe that $\comsubtype S T$ hold if and only if $\alg {\t{compat}} {\t{covariant}} S T$.

Also, $\matches S T$ if and only if $\alg {\t{bicompat}} {\t{covariant}} S T$.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\begin{displayfigure}{th}{Type Algorithms}
\footnotesize
\label{fig:alg}
\[
\begin{array}{rcl}
	b &::=& \t{bicompat}~|~\t{compat} \\
	v &::=& \t{covariant}~|~\t{invariant}
\end{array}
\]
\begin{trules}
\headtrule{$\alg{b}{v}{S}{T}$}{Matching Algorithm}
\trule{A-Int}{
	}{
		\alg b v  \Int \Int
	}
...
\trule{A-Arrow}{
		%\subtype{T_1}{S_1} \qquad
		\alg b {\t{invariant}} {S_1}{T_1} \qquad
		\alg b v {S_2}{T_2}
	}{
		\alg b v {(\lamt{S_1}{S_2})}{(\lamt{T_1}{T_2})}
	}
\trule{A-Obj}{
		%\subtype{T_i}{S_i}\qquad
		%\subtype{S_i}{T_i} 
		\alg b {\t{invariant}} {S_i}{T_i} \qquad\mbox{for $i\in 1..n$}
	}{
		\alg b v {\objty{l_i:S_i^{i\in 1..n+m}}}
				{\objty{l_i:T_i^{i\in 1..n}}}
	}
\trule{A-Generic}{
		\alg b v  S T 
	}{
		\alg b v {\Forall X S}{\Forall X T}
	}
\trule{A-Like-Inc}{
		\alg b {\t{covariant}}  S T 
	}{
		\alg b {\t{covariant}} S {\likety T}
	}
\trule{A-Like-Cov}{
		\alg b v  S T 
	}{
		\alg b v  {\likety S} {\likety T}
	}
\trule{A-Wrap}{
		\alg b {\t{covariant}}  S T
	}{
		\alg b {\t{covariant}} {\wrapty S} T
	}
\trule{A-Wrap-Cov}{
		\alg b v  S T 
	}{
		\alg b v  {\wrapty S} {\wrapty T}
	}
\trule{A-Int-Bool}{
	}{
		\alg {\t{bicompat}} {\t{covariant}}  \Int \Bool
	}
\end{trules}
\end{displayfigure}
...
\begin{displayfigure}{th}{Compatible-Subtyping Algorithm}
\footnotesize
\label{fig:alg}
\begin{trules}
\headtrule{$\comsubtype{S}{T}$}{Compatible-Subtyping}
\trule{CS-Int}{
	}{
		\comsubtype \Int \Int
	}
...
\trule{CS-Arrow}{
		%\subtype{T_1}{S_1} \qquad
		\comeqtype{S_1}{T_1} \qquad
		\comsubtype{S_2}{T_2}
	}{
		\comsubtype{(\lamt{S_1}{S_2})}{(\lamt{T_1}{T_2})}
	}
\trule{CS-Obj}{
		%\subtype{T_i}{S_i}\qquad
		%\subtype{S_i}{T_i} 
		\comeqtype{S_i}{T_i} \qquad\mbox{for $i\in 1..n$}
	}{
		\comsubtype{\objty{l_i:S_i^{i\in 1..n+m}}}
				{\objty{l_i:T_i^{i\in 1..n}}}
	}
\trule{CS-Generic}{
		\comsubtype S T 
	}{
		\comsubtype{\Forall X S}{\Forall X T}
	}
\trule{CS-Like-Inc}{
		\comsubtype S T 
	}{
		\comsubtype S {\likety T}
	}
\trule{CS-Like-Cov}{
		\comsubtype S T 
	}{
		\comsubtype {\likety S} {\likety T}
	}
\trule{CS-Wrap}{
		\comsubtype S T
	}{
		\comsubtype {\wrapty S} T
	}
\trule{CS-Wrap-Cov}{
		\comsubtype S T 
	}{
		\comsubtype {\wrapty S} {\wrapty T}
	}
\headtrule{$\comeqtype{S}{T}$}{Compatible-Equality}
\trule{CE-Int}{
	}{
		\comeqtype \Int \Int
	}
...
\trule{CE-Arrow}{
		...
		\comeqtype{(\lamt{S_1}{S_2})}{(\lamt{T_1}{T_2})}
	}
\trule{CE-Obj}{
		%\subtype{T_i}{S_i}\qquad
		%\subtype{S_i}{T_i} 
		...
		\comeqtype{\objty{l_i:S_i^{i\in 1..n}}}
				{\objty{l_i:T_i^{i\in 1..n}}}
	}
\trule{CE-Generic}{
		\comeqtype S T 
	}{
		\comeqtype{\Forall X S}{\Forall X T}
	}
\trule{CE-Like}{
		\comeqtype S T 
	}{
		\comeqtype {\likety S} {\likety T}
	}
\trule{CE-Wrap}{
		\comeqtype S T 
	}{
		\comeqtype {\wrapty S} {\wrapty T}
	}
\end{trules}
\end{displayfigure}


\begin{displayfigure}{th}{Matching Algorithm (work in progress)}
\footnotesize
\label{fig:alg}
\begin{trules}
\headtrule{$\matches{S}{T}$}{Matching Algorithm}
\trule{M-Int}{
	}{
		\matches \Int \Int
	}
...
\trule{M-Arrow}{
		...
		\matches{(\lamt{S_1}{S_2})}{(\lamt{T_1}{T_2})}
	}
\trule{M-Obj}{
		%\subtype{T_i}{S_i}\qquad
		%\subtype{S_i}{T_i} 
		...
		\matches{\objty{l_i:S_i^{i\in 1..n+m}}}
				{\objty{l_i:T_i^{i\in 1..n}}}
	}
\trule{M-Generic}{
		\matches S T 
	}{
		\matches{\Forall X S}{\Forall X T}
	}
\trule{M-Like-Inc}{
		\matches S T 
	}{
