...
The type $\dynamic$ is a top type. Although the subtyping judgement does not include an explicit environment, the rule \rel{S-Generic} means that the types may include free type variables. This rule applies implicit alpha-renaming so that the same type variable can be interpreted as the same on both sides.
...
We have that $\subtype{\likety{\likety{T}}}{\likety{T}}$ and hence $\compatible{\likety{\likety{T}}}{\likety{T}}$,
and we also have that $\compatible{\likety{T}}{\likety{\likety{T}}}$, but
 $\subtype{\likety{T}}{\likety{\likety{T}}}$ does not hold.
