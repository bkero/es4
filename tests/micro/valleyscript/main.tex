...
\mbox{Includes the above subtyping rules (with $\!\!\!\!\subtype{}{}\!\!\!\!$ replaced by $\!\compatible{}{}$), and also:}\\
\trule{Com-Dyn}{
	}{
...
can be assigned to a variable of type $T$ (some caveats for \t{like} types are needed here).
...
The rule~\rel{Sub-Like} for \t{like} types switches from checking subtyping to checking compatibility.
...
\begin{array}{rcll}
	...
	\convert{v}{\wrapty T} &=&  \wrapv{v}{T}		&\mbox{if $\istype{v}{\likety T}$}
\end{array}
\]
